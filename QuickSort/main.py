...
def quick_sort(lista, ini, fim):
    if ini < fim:
        pivo = pivo_aleatorio(lista, ini, fim)
        quick_sort(lista, ini, pivo)
        quick_sort(lista, pivo + 1, fim)
    return lista

def pivo_aleatorio(lista, ini, fim):
...
for i in range(len(tam)):
    ...
    times.append(timeit.timeit("quick_sort({}, {}, {})".format(lista_invertida, 0, len(lista_invertida)),
                               setup="from __main__ import quick_sort, pivo_aleatorio, particao", number=1))

desenha_grafico(tam, times, "Tempo.png", "Tempo gasto pelo quick_sort", xl="Tamanho da lista", yl="Tempo")
